"""
Verify Phase B (memory): run the same brief through the pipeline twice, each
in a fresh session with the bi-pipeline-lessons memory store attached
(mirroring what session/new/route.js now does), and check whether
bi-design/bi-authoring actually read and write lessons across the two runs.
